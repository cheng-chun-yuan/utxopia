...
describe("Constants", () => {
  test("DEFAULT_COMPUTE_UNITS is reasonable", () => {
    ...
    expect(DEFAULT_COMPUTE_UNITS).toBeLessThan(1_400_000);
  });
...
describe("encodeSetComputeUnitLimit", () => {
  test("encodes correctly", () => {
    ...
    expect(data[0]).toBe(
      COMPUTE_BUDGET_DISCRIMINATORS.SET_COMPUTE_UNIT_LIMIT,
    );
...
describe("encodeSetComputeUnitPrice", () => {
  test("encodes correctly", () => {
    ...
    expect(data[0]).toBe(
      COMPUTE_BUDGET_DISCRIMINATORS.SET_COMPUTE_UNIT_PRICE,
    );
...
describe("buildPriorityFeeInstructionData", () => {
  test("builds instruction data without API", async () => {
    ...
    expect(result.setComputeUnitPrice?.microLamports).toBe(
      BigInt(DEFAULT_PRIORITY_FEE),
    );
  });
