...
#   2. privacy-coin-app/.env.{network}        + symlink privacy-coin-app/.env.local
#   3. privacy-coin-app/src/lib/networks.json (frontend config)
#
# Usage:
...
NETWORK="${PRIVACY_COIN_NETWORK:-localnet}"

echo "=== Privacy Coin Config Sync ==="
...
cat > "$BACKEND_ENV" << EOF
# Privacy Coin Backend — $NETWORK (auto-generated by sync-env.sh)
# Source: $STATE_FILE
# Re-generate: PRIVACY_COIN_NETWORK=$NETWORK ./scripts/sync-env.sh

# ─── Solana ──────────────────────────────────────────────────────────────────
PRIVACY_COIN_PROGRAM_ID=$PROGRAM_ID
PRIVACY_COIN_ZKBTC_MINT=$ZKBTC_MINT
PRIVACY_COIN_NETWORK=$NETWORK
PRIVACY_COIN_SOLANA_RPC=$SOLANA_RPC
SOLANA_RPC_URL=$SOLANA_RPC
BTC_LIGHT_CLIENT_PROGRAM_ID=$BTC_LC_ID

# ─── Bitcoin ─────────────────────────────────────────────────────────────────
PRIVACY_COIN_BITCOIN_NETWORK=$BTC_NETWORK
PRIVACY_COIN_BITCOIN_RPC=$ESPLORA_URL
...
# ─── 2. Generate privacy-coin-app/.env.{network} ────────────────────────────────────

FRONTEND_ENV="$ROOT/privacy-coin-app/.env.$NETWORK"
cat > "$FRONTEND_ENV" << EOF
# Privacy Coin Frontend — $NETWORK (auto-generated by sync-env.sh)
# Source: $STATE_FILE
# Re-generate: PRIVACY_COIN_NETWORK=$NETWORK ./scripts/sync-env.sh
...
NEXT_PUBLIC_PRIVACY_COIN_PROGRAM_ID=$PROGRAM_ID
...
ln -sf ".env.$NETWORK" "$ROOT/privacy-coin-app/.env.local"
echo "✓ Symlinked privacy-coin-app/.env.local → .env.$NETWORK"

# ─── 3. Generate privacy-coin-app/src/lib/networks.json ─────────────────────────────
#
# This is the frontend's runtime config. We update ONLY the current network's
# section, preserving other networks' config.

NETWORKS_JSON="$ROOT/privacy-coin-app/src/lib/networks.json"
...
echo "  PRIVACY_COIN_NETWORK=devnet ./scripts/sync-env.sh"
