...
stop_all() {
  ...
  for pidfile in /tmp/utxopia-frontend.pid /tmp/utxopia-backend.pid /tmp/utxopia-validator.pid; do
    if [ -f "$pidfile" ]; then
...
# UTXOpia Frontend — Devnet Configuration
NEXT_PUBLIC_NETWORK=devnet
NEXT_PUBLIC_SOLANA_RPC_URL=https://api.devnet.solana.com
NEXT_PUBLIC_BTC_NETWORK=testnet4
NEXT_PUBLIC_UTXOPIA_PROGRAM_ID=8fqRet9WB5PECvKfWmzTPSusJgQz1onzxTLfHD75XKim
TRACKER_API_URL=https://api-aegis.amidoggy.xyz
BACKEND_URL=https://api-aegis.amidoggy.xyz
BACKEND_API_KEY=utxopia-backend-2026
EOF
...
setup_mainnet() {
  ...
  cat > web/.env.local << 'EOF'
# UTXOpia Frontend — Mainnet Configuration (read-only)
NEXT_PUBLIC_NETWORK=mainnet
NEXT_PUBLIC_SOLANA_RPC_URL=https://api.mainnet-beta.solana.com
NEXT_PUBLIC_BTC_NETWORK=mainnet
# NEXT_PUBLIC_UTXOPIA_PROGRAM_ID=  # Not deployed yet
...
setup_localnet() {
  ...
  UTXOPIA_SO="contracts/target/deploy/utxopia.so"
  BTC_LC_SO="contracts/target/deploy/btc_light_client.so"
  if [ ! -f "$UTXOPIA_SO" ]; then
    err "UTXOpia program not built. Run: cd contracts && cargo build-sbf --features devnet"
    exit 1
  fi
  if [ ! -f "$BTC_LC_SO" ]; then
  ...
  cat > web/.env.local << EOF
# UTXOpia Frontend — Localnet (auto-generated by setup.sh)
