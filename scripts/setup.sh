...
setup_devnet() {
  ...
  cat > privacy-coin-app/.env.local << 'EOF'
...
EOF
  ...
  log "Written: privacy-coin-app/.env.local"
  log ""
  log "Devnet configured. Run: cd privacy-coin-app && bun run dev"
...
EOF
  log "Written: privacy-coin-app/.env.local (zkBTC: ${ZKBTC_MINT:-?}, USDC: ${USDC_MINT:-?}, wSOL: ${WSOL_MINT:-?})"
