...
async function verifyDeposit(
  ...
  const [depositReceiptPDA] = PublicKey.findProgramAddressSync(
    [Buffer.from("deposit_receipt"), Buffer.from(depositTxidInternal)], UTXOPIA_PROGRAM_ID
  );
...
async function main() {
  ...
  console.log("Program:", UTXOPIA_PROGRAM_ID.toBase58());
