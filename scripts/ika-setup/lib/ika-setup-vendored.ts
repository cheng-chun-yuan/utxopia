...
export type CurveName = "Secp256k1" | "Curve25519";

function curveBcs(name: CurveName) {
  return name === "Secp256k1" ? { Secp256k1: true } : { Curve25519: true };
}

function curveByte(name: CurveName): number {
  return name === "Secp256k1" ? CURVE_SECP256K1 : CURVE_CURVE25519;
}
...
function loadGrpcClient(grpcUrl: string): any {
  ...
  // The upstream helper uses createInsecure() which works against the
  // local mock server but not the production :443 endpoint. Switch to
  // TLS when the URL is on port 443 (or explicitly https://).
  const stripped = grpcUrl.replace(/^https?:\/\//, "");
  const useTls = grpcUrl.startsWith("https://") || stripped.endsWith(":443");
  const creds = useTls
...
export async function setupDWallet(
  ...
  const [dwalletPda] = pda(
    dwalletPdaSeeds(curveByte(curve), publicKey),
    dwalletProgramId,
  );
