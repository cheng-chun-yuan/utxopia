...
async function main() {
  ...
  const [depositReceipt] = PublicKey.findProgramAddressSync([Buffer.from("deposit_receipt"), depositTxHash], AEGIS);
  const voutBuf = Buffer.alloc(4); voutBuf.writeUInt32LE(0);
  const [utxoRecord] = PublicKey.findProgramAddressSync([Buffer.from("utxo"), sweepTxHash, voutBuf], AEGIS);
