...
async function main() {
  ...
  // Start Surfpool offline with auto-deploy via txtx runbook.
  // Requires: contracts/txtx.yml + runbooks/ (auto-generated by surfpool -y)
  ...
  for (const [src, dst] of symlinkPairs) {
    const dstPath = path.join(targetDir, dst);
    if (!fs.existsSync(dstPath)) {
      fs.symlinkSync(src, dstPath);
    }
  }
  ...
  execSync(`nohup ${surfpoolCmd} > /tmp/surfpool.log 2>&1 &`, {
    shell: "/bin/bash",
    cwd: CONTRACTS_DIR,
  });
  ...
  if (fs.existsSync(chadbufferSoLocal) && fs.existsSync(chadbufferKpPath)) {
    execSync(
      `solana program deploy ${chadbufferSoLocal} --program-id ${chadbufferKpPath} --url http://localhost:8899 -k ~/.config/solana/id.json`,
      { stdio: "pipe" },
    );
  }
  log(`ChadBuffer loaded at ${chadbufferId.toBase58()}`);
  ...
  const nativeMintInfo = await devnetConn.getAccountInfo(new PublicKey(NATIVE_MINT_2022));
  if (nativeMintInfo) {
    await fetch("http://127.0.0.1:8899", {
      ...
      body: JSON.stringify({
        ...
        params: [NATIVE_MINT_2022, {
          lamports: nativeMintInfo.lamports,
          data: Buffer.from(nativeMintInfo.data).toString("hex"),
          owner: nativeMintInfo.owner.toBase58(),
          executable: nativeMintInfo.executable,
        }],
      }),
    });
  }
