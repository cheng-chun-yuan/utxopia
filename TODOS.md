...
- [x] **Per-stealth-address compliance toggle**: SNS subdomain records carry an optional `complianceFlags: u8` byte (bit 0 = AUDITOR_DISCLOSABLE). SDK reader is back-compat with legacy 65-byte records (flags default to 0); Send-form preview-resolves the SNS name and renders an "Auditor-disclosable" chip when the bit is set. Owners set / clear via `scripts/sns-set-compliance.ts <subdomain> --enable|--disable`.
  - [ ] **Settings page UI** to flip the flag without the CLI (currently CLI-only).
