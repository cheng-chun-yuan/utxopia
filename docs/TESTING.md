...
- Frontend tests are co-located with code using Bun test:
  - Stores: `src/stores/__tests__/utxopia-store.test.ts`, `src/stores/__tests__/notes-store.test.ts`.
...
How to run the full UTXOpia stack on **localnet** (regtest) and **devnet** (testnet4), including FROST threshold signing.
...
1. **Demo deposit** — admin instruction, instant, creates shielded notes:
   ```bash
   bun run scripts/topup-all.ts utxo:<stealth_meta_address>
   ```
...
| `utxopia-app/.env.localnet` | `utxopia-app/.env.local` |
...
| Backend says `UTXOPIA_PROGRAM_ID required` | Run `./scripts/sync-env.sh` then start from `backend/` directory |
