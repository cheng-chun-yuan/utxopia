...
vi.mock("@/lib/constants", () => ({
  AEGIS_PROGRAM_ID: "MockProgramId1111111111111111111111111111111",
}));

import { useAegistore, useAAEGISuseAeAEGISs, useStealthInbox } from "../aegis-store";

describe("useAegisStore", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Reset store state
  ...
  describe("initial state", () => {
    it("starts with correct default values", () => {
      const { result } = renderHook(() => useAegisStore());
  ...
  describe("useAegis convenience hook", () => {
    it("returns full store state", () => {
      const { result } = renderHook(() => useAegis());
  ...
  describe("useAegisKeys convenience hook", () => {
    it("returns keys-related state only", () => {
      const { result } = renderHook(() => useAegisKeys());
