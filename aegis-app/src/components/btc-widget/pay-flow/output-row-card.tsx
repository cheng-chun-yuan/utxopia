...
function RecipientInput({ output, onUpdate, defaultAddress, selfMeta }: {
  ...
  if (output.mode === "note") {
    ...
    const urlPrefix = `${origin}/claim#note=`;
    return (
      <div>
        <p className="text-[11px] font-mono text-gray/40 mb-1 pl-1 truncate">{urlPrefix}</p>
        <div className="relative">
        ...
        {output.secretPhrase.trim().length > 0 && output.secretPhrase.trim().length < 8 && (
          <p className="text-[10px] text-gray mt-0.5 pl-2">
            {8 - output.secretPhrase.trim().length} more chars needed
          </p>
        )}
      </div>
    );
